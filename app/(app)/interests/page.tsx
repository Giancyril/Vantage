...
import { Sliders, Plus, Trash2, RotateCcw, Check, Sparkles, ExternalLink } from "lucide-react";
...
export default function InterestsPage() {
  ...
  const loadCurated = async () => {
    try {
      const res = await fetch("/api/interests?curated=true");
      const data = await res.json();
      if (data.curatedTopics) setCurated(data.curatedTopics);
    } catch (e) {
      console.error(e);
    }
  };
    ...
    <div className="max-w-3xl mx-auto space-y-8">
      ...
      <div className="space-y-4">
        ...
        <div className="space-y-3">
          {interests.map((item) => (
            <div
              key={item.id}
              className="bg-white rounded-xl border border-[#E9E5DE] p-5 transition-all hover:border-[#D0CAC0]"
            >
                ...
                <div className="space-y-1.5 flex-1">
                  ...
                  <div className="flex flex-wrap gap-1.5 pt-1">
                    {item.keywords.map((kw, idx) => (
                      <span
                        key={idx}
                        className="text-xs px-2.5 py-0.5 rounded-md bg-[#F4EFE7] text-[#4A453E] border border-[#E9E4DC]"
                      >
                        {kw}
                      </span>
                    ))}
              ...
              <div className="mt-4 pt-3 border-t border-[#F4EFE7] flex items-center space-x-4">
                <span className="text-[11px] font-medium text-[#8A8378] uppercase tracking-wider shrink-0">
                  Priority Weight:
                </span>
                <input
                  type="range"
                ...
                />
                <span className="text-xs font-semibold text-[#181715] w-8 text-right">
      ...
      <div className="bg-white rounded-xl border border-[#E9E5DE] p-6 space-y-4">
        ...
        <form onSubmit={handleAddCustom} className="space-y-4">
          <div>
            <label className="block text-xs font-semibold uppercase tracking-wider text-[#8A8378] mb-1">
              Domain / Topic Title
            </label>
            <input
              type="text"
          ...
          <div>
            <label className="block text-xs font-semibold uppercase tracking-wider text-[#8A8378] mb-1">
              Key Concepts & Phrases (Comma separated)
            </label>
            <input
              type="text"
          ...
          <div className="flex justify-end">
            <button
              ...
              className="px-4 py-2 rounded-full bg-[#181715] text-white text-xs font-semibold hover:bg-[#33302B] transition-all disabled:opacity-50 cursor-pointer"
            >
              Add Domain to Feed
            </button>
          </div>

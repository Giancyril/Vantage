...
export default function SavedStoriesPage() {
  ...
  const loadSaved = async () => {
    try {
      ...
      // Discover current articles to match details
      ...
      const matched = articles.filter((a: any) => urls.includes(a.url));
      ...
      {savedStories.length === 0 ? (
        <div className="py-16 text-center bg-white rounded-xl border border-[#E9E5DE] p-8 space-y-3">
          ...
          <p className="text-xs text-[#6E675C] max-w-sm mx-auto">
            Click "Save for Later" on any card in your Daily Feed to curate your personal archive.
          </p>
        </div>
      ) : (
        <div className="space-y-4">
          {savedStories.map((story) => (
            <div
              key={story.url}
              className="bg-white rounded-xl border border-[#E9E5DE] p-5 sm:p-6 transition-all hover:border-[#D5CFC5]"
            >
              ...
              <p className="text-sm text-[#4A453E] leading-relaxed mb-3">
                {story.snippet}
              </p>
